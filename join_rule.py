...
def match_on_conditions(  # type: ignore
    ...
    #print(*list(map(lambda terms: evaluate_condition(*terms), condition_terms)))

    not_in = None
    evaluated_conditions = []
    for terms in condition_terms:
        #We need to prioritize the NOT IN
        if(terms[4] == JoinOperator.NOT_IN):
            not_in = terms
            continue
        #if(terms)
    # Step 3: Unpack the tuple and call evaluate_condition with the unpacked values
        result = evaluate_condition(*terms)
    # Step 4: Add the result to our list of evaluated conditions
        evaluated_conditions.append(result)
...
def run_join_rule(
    ...
    #print(collection_set_unaliased)
